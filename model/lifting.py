...
class Wavelet(nn.Module):
    ...
    该模块使用pywt中定义的小波系数并创建2D卷积核，以便能够使用GPU
    """
...
if __name__ == "__main__":
    ...
    # m_wavelet = Wavelet2D(3, name="db2")
    m_wavelethaar = WaveletHaar2D()
    m_ls = LiftingScheme2D(3, share_weights=True, modified=True, size=[2, 1], kernel_size=4)
    # print(input)
    ...
    # 如何调用LiftingScheme2D
    # input = torch.randn(1, 1, 10, 10)
    # m_ls = LiftingScheme2D(1, share_weights=False, modified=True, size=[2, 1], kernel_size=4)
    # output = m_ls(input)
    # print(output)
    # print(len(output))
