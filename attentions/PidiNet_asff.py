...
class PiDiNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_asff0 = asff_0(x3, x2, x1)
        out_asff1 = asff_1(x3, x2, x1)
        out_asff2 = asff_2(x3, x2, x1)

        e1 = self.conv_reduces[0](out_asff0)
        e2 = self.conv_reduces[1](out_asff1)
        e3 = self.conv_reduces[2](out_asff2)
...
if __name__ == '__main__':
    ...
    model = PiDiNet(3, 'rd').to(device)     # cv cd ad rd
    out = model(img)
    for i, result in enumerate(out):
        result = result.squeeze(0)
        result = TF.to_pil_image(result)
        result.save(f'./result_rd_{i}.png')